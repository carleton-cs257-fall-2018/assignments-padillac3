...
class BooksDataSource:
    ...
    def create_books_list(self, books_filename):
        ...
        for book in books_csv:
            book_dictionary = {'id': int(book[0]), 'title':book[1], 'publication_year': int(book[2])}
    ...
    def create_authors_list(self, authors_filename):
        ...
        for author in authors_csv:
            if author[4] == "NULL":
                author_dictionary = {'id': int(author[0]), 'last_name': author[1], 'first_name': author[2], 'birth_year': int(author[3]), 'death_year': None}
            else:
                author_dictionary = {'id': int(author[0]), 'last_name': author[1], 'first_name': author[2], 'birth_year': int(author[3]), 'death_year': int(author[4])}
    ...
    def _sort_first_title_then_year(self, book):
        return (book['title'], book['publication_year'])
    ...
    def books_with_start_year(self, start_year, books_list):
        if type(start_year) != int:
            ...
            for book in books_list:
                if book['publication_year'] - start_year >= 0:
    ...
    def books_with_end_year(self, end_year, books_list):
        if type(end_year) != int:
            ...
            book_list = []
            for book in books_list:
                if book['publication_year'] - end_year <= 0:
                    book_list.append(book)
            return book_list
    ...
    def authors_with_search_text(self, search_text, result_list=None):
        ...
        for author in result_list:
            if author in author_list:
                pass
            elif search_text.lower() in author['last_name'].lower():
                author_list.append(author)
            elif search_text.lower() in author['first_name'].lower():
                author_list.append(author)
        return(author_list)
